...
def render_import_page() -> None:
    st.title("Bewerberzahlen importieren")
    st.markdown(
        "Bereinigt Bewerber-CSV-Dateien: Dubletten filtern, Status ableiten, Fachbereiche "
        "zuordnen, personenbezogene Spalten entfernen und bereinigte Daten speichern."
    )
    st.info("Maximale Upload-Größe: 20 MB", icon="ℹ️")

    with st.form("bereinigung_form"):
        uploader = st.file_uploader(
            "CSV-Datei hochladen", type=["csv"], accept_multiple_files=False
        )
    ...
    if isinstance(uploaded_bytes, (bytes, bytearray)) and isinstance(uploaded_name, str):
        _render_processing_result(bytes(uploaded_bytes), uploaded_name)


def _render_processing_result(uploaded_bytes: bytes, uploaded_name: str) -> None:
    try:
        df = read_import_csv_from_bytes(uploaded_bytes)
    ...
    if result.errors:
        ...
        st.warning("Bereinigung pausiert: Bitte Dubletten-Auswahl vollständig ausfüllen.", icon="⚠️")
    else:
        st.success(
            "Bereit: "
            f"Eingelesen: {result.n_input}, behalten: {result.n_kept}, "
            f"Dubletten: {result.n_duplicates}, fehlender Studiengang ignoriert: "
            f"{result.n_missing_program}, unbekannte Studiengänge: {result.n_unknown_program}"
        )
    ...
    if result.cleaned is None:
        ...
        _render_cleaned_download_and_import(result.cleaned, uploaded_name)
...
def _render_cleaned_download_and_import(cleaned: pd.DataFrame, uploaded_name: str) -> None:
    today = date.today().strftime("%Y%m%d")
    base_name = Path(uploaded_name).stem

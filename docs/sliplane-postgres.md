...
Diese Anleitung beschreibt die produktive Datenbank für Phase 2.

## Zielbild

- PostgreSQL läuft als privater Sliplane-Service.
- Die Streamlit-App greift über `DATABASE_URL` auf PostgreSQL zu.
...
2. `Deploy Service` wählen.
3. Als Deploy Source ein vorkonfiguriertes Docker-Hub-Image für PostgreSQL wählen.
...
10. Mount Path für das Volume: `/var/lib/postgresql/data`.

## Environment Variables für PostgreSQL
...
Nach dem Deploy im PostgreSQL-Service unter `Service Settings` den internen Host bzw. die interne TCP-Adresse kopieren. Private Services können laut Sliplane-Doku von anderen Services auf demselben Server über diesen internen Host erreicht werden.
...
```env
DATABASE_URL=postgresql://bewerberzahlen_app:<starkes-passwort>@<internal-host>:5432/bewerberzahlen
IMPORT_DELETE_PASSWORD=<starkes-lösch-passwort>
...
Das Löschen gespeicherter Importe ist nur verfügbar, wenn `IMPORT_DELETE_PASSWORD` gesetzt ist.
...
Die Sliplane-Produktivdatenbank sollte für lokale Entwicklung nicht öffentlich exponiert werden.
